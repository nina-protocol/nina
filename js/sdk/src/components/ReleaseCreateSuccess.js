...
const ReleaseCreateSuccess = (props) => {
  ...
  const { releaseName, hubHandle, inHubs } = props
  ...
  const shareOnTwitter = () => {
    window.open(
      `https://twitter.com/intent/tweet?text=${`${activeTrack.current.properties.artist} - "${activeTrack.current.properties.title}" on Nina%0A`}&url=ninaprotocol.com/${
