    ...
    try {
      ...
      if (!isSol(release.paymentMint) && usdcBalance < ninaClient.nativeToUi(release.price.toNumber(), ids.mints.usdc)) {
        ...
        const { data } = await axios.get(
          `https://quote-api.jup.ag/v1/quote?inputMint=So11111111111111111111111111111111111111112&outputMint=EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v&amount=${ninaClient.uiToNative((releaseUiPrice + (releaseUiPrice * .02)) / solPrice, ids.mints.wsol)}&slippage=0.5&onlyDirectRoutes=true`
        )
  ...
  const releasePurchase = async (releasePubkey) => {
    ...
    try {
      ...
      if (!isSol(release.paymentMint) && usdcBalance < ninaClient.nativeToUi(release.price.toNumber(), ids.mints.usdc)) {
        ...
        const releaseUiPrice = ninaClient.nativeToUi(release.price.toNumber(), ids.mints.usdc) - usdcBalance
        const { data } = await axios.get(
          `https://quote-api.jup.ag/v1/quote?inputMint=So11111111111111111111111111111111111111112&outputMint=EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v&amount=${ninaClient.uiToNative((releaseUiPrice + (releaseUiPrice * .02)) / solPrice, ids.mints.wsol)}&slippage=0.5&feeBps=50&onlyDirectRoutes=true`
        )
        const transactions = await axios.post(
          'https://quote-api.jup.ag/v1/swap', {
          route: data.data[0],
